...
{
    public class WeatherStationGenerator : DataGenerator<WeatherStation>
    {
        public WeatherStationGenerator(int collectionLength)
        {
            this.PopulateCollection(collectionLength);
        }

        public static WeatherStationGenerator GetInstance(int collectionLenght)
        {
            return new WeatherStationGenerator(collectionLenght);
        }
        ...
        public override void PopulateCollection(int? collectionLength = null)
        {
            ...
            var collection = new List<WeatherStation>();
            for (int i = 0; i < length; i++)
            {
                collection.Add(WeatherStationFactory.Instance.CreateFakeItem(i));
            }
